...
describe('TextColoredLinksComponent', () => {
  ...
  describe('ngOnChanges', () => {
    ...
    it('should infoGraphicItems length be same as Color text items length', () => {
      expect(component.infoGraphicItems.length).toEqual(
        COLOR_TEXT_ITEMS.length
      );
    });
    it('should text in infoGraphicItems remain the same', () => {
      expect(component.infoGraphicItems[0].label).toEqual(
        COLOR_TEXT_ITEMS[0].label
      );
    });
  });
  describe('onItemClick', () => {
    ...
    it('should onItemClick run action if has one', () => {
      ...
      expect(infoGraphicItemsMock[0].action).toHaveBeenCalledWith(
        infoGraphicItemsMock[0]
      );
    });
